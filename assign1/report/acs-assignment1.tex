...
forward and follows the style used by the features already implemented with the assignment

The all-or-nothing semantics are enforced by 
...
\item{Several naming mechanisms are used by the architecture, at multiple
    ...
    using TCP/IP relies on another naming scheme, namely IP addresses with each IP corresponding to the network interface of a system connected to the internet.}
